/* 1001 Classical — 单页应用，hash 路由，纯前端渲染。以「录音」为单位。
 * 数据字段：title=作品英文名(搜索用) · zh=中文曲名 · artist=作曲家(英文键) · perf=演绎 · year · label · era · genres · instruments · moods · reason */
(function(){
  "use strict";
  ...
  const workZh = a => a.zh || a.title;                       // 展示用中文曲名
  const composerZh = c => (BIOS[c] && BIOS[c].zh) || c;      // 作曲家中文名
  const perfShort = a => String(a.perf||"").split("·")[0].trim();

  function hashStr(s){let h=0;for(let i=0;i<s.length;i++){h=(h<<5)-h+s.charCodeAt(i);h|=0;}return Math.abs(h);}
  // 古典厂牌配色（唱片脊色调）
  const LABEL_THEME = {
    "Deutsche Grammophon":["#f5d76e","#1a1205"], "Decca":["#7a1414","#2a0808"],
    "EMI":["#3a2d5e","#120e22"], "Philips":["#1c3a5e","#0d1b2e"],
    "Columbia":["#5a1f1f","#1f0a0a"], "CBS":["#5a1f1f","#1f0a0a"],
    "RCA":["#5e2b2b","#1f0e0e"], "Sony":["#20303f","#0b1218"],
    "ECM New Series":["#2b3a3f","#0e1416"], "Nonesuch":["#244a2c","#0c1a10"],
    "Hyperion":["#2d2466","#120f2e"], "Gimell":["#4a3d18","#1c1608"],
    "Archiv":["#4a3418","#1a1206"], "Harmonia Mundi":["#5a3d18","#1f1408"],
    "Erato":["#5a2440","#200d18"], "Teldec":["#2a3d63","#0d1322"],
    "L'Oiseau-Lyre":["#3d5a24","#141f0c"], "Melodiya":["#7a1414","#2a0808"],
    "Warner":["#3a2d5e","#120e22"], "Seminary":["#333","#111"]
  };
  const PLAY_ICONS = {
  ...
  function coverHTML(a,big,quick){
    ...
    else{const h=hashStr(a.artist+a.title)%360; c1=`hsl(${h} 34% 26%)`; c2=`hsl(${(h+40)%360} 40% 10%)`;}
    const accent = `hsl(${(hashStr(a.title)*7)%360} 55% 58%)`;
    const fs = big?"1.7rem":"1.05rem";
    let overlay = "";
    if(quick){
    ...
    return `<div class="cover" data-album="${a.id}" style="background:linear-gradient(150deg,${c1},${c2})">
      ...
      <span class="c-composer">${esc(composerZh(a.artist))}</span>
      <span class="c-year">${a.year}</span>
      <img class="cover-img" alt="${esc(workZh(a))} — ${esc(a.artist)} 唱片封面" loading="lazy">
      ${overlay}
    </div>`;
  }

  /* ---------- 真实唱片封面：iTunes Search API（entity=album，客户端 JSONP + 缓存，失败回退程序化封面） ---------- */
  const coverCache = {};
  let jsonpSeq = 0;
  function lsGet(k){try{return localStorage.getItem(k);}catch(e){return null;}}
  function lsSet(k,v){try{localStorage.setItem(k,v);}catch(e){}}
  // 搜索词：作曲家 + 作品英文名 + 主要演绎者（指挥/独奏），最贴近古典唱片在 iTunes 的命名
  function albumQuery(a){
    let t = a.title.replace(/\s*\([^)]*\)\s*$/,"").replace(/\s*['’][^'’]*['’]\s*$/,"").trim();
    return [a.artist, t, perfShort(a)].filter(Boolean).join(" ");
  }
  function pickArtwork(album,data){
    if(!data||!data.results||!data.results.length) return "";
    const norm=x=>String(x||"").toLowerCase().replace(/[^a-z0-9]/g,"");
    const wantA=norm(album.artist), wantT=norm(album.title), wantP=norm(perfShort(album));
    let best=data.results[0],score=-1;
    data.results.forEach(r=>{
      const a=norm(r.artistName), t=norm(r.collectionName||r.trackName);
      let sc=0;
      if(wantA&&(a.includes(wantA)||t.includes(wantA))) sc+=2;
      if(wantT&&t.includes(wantT.slice(0,10))) sc+=3;
      if(wantP&&(a.includes(wantP)||t.includes(wantP))) sc+=2;
  ...
  function loadCover(album,imgEl){
    ...
    const key="ccov1:"+album.id, cached=lsGet(key);
    if(cached){ coverCache[album.id]=cached; applyCover(imgEl,cached); return; }
    const cbName="__clcb"+(jsonpSeq++);
    ...
    sc.onerror=()=>finish("");
    sc.src=`https://itunes.apple.com/search?term=${term}&entity=album&limit=10&callback=${cbName}`;
    document.body.appendChild(sc);
    setTimeout(()=>finish(""),9000);
  }
  // 懒加载：仅在封面进入视口附近时才请求 iTunes
  ...
  /* ---------- 作曲家肖像：维基百科 pageimages（JSONP + localStorage 缓存，失败回退字母徽章） ---------- */
  const portraitCache = {};
  ...
  function flushPortraitQueue(){
    ...
    const finish=map=>{ if(done)return; done=true; try{delete window[cbName];}catch(e){} sc.remove();
      batch.forEach(it=>{ const url=map[it.title]||""; portraitCache[it.name]=url; lsSet("cart1:"+it.name,url); applyPortrait(it.imgEl,it.medalEl,url); }); };
    window[cbName]=data=>{
      const map={};
      try{
        const q=data.query||{};
  ...
  const PLATFORMS = {
    ...
    idagio:{n:"IDAGIO",color:"#c9a24a",ic:'<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 100 20 10 10 0 000-20zm0 3a7 7 0 110 14 7 7 0 010-14zm0 3a4 4 0 100 8 4 4 0 000-8z"/></svg>'},
  ...
  function listenLinks(a){
    const q = enc(`${a.artist} ${a.title} ${perfShort(a)}`);
    const U = {
      ...
      idagio:`https://app.idagio.com/search?q=${q}`,
      douban:`https://search.douban.com/music/subject_search?search_text=${q}`
    };
    return ["netease","qq","spotify","apple","youtube","idagio","douban"]
  ...
  function albumCard(a){
    return `<article class="song-card" data-y="${a.year}" data-b="${born(a.artist)}" data-e="${eraIdx[a.era]}" data-c="${esc(a.artist)}" data-t="${esc(workZh(a))}" onclick="location.hash='#/album/${a.id}'">
      ...
      <div class="meta">
        ...
        <div class="s-artist">${esc(composerZh(a.artist))}</div>
        <div class="s-perf">${esc(a.perf)}</div>
  ...
  function featuredArtistsModule(){
    ...
    return `<section class="section">
      <div class="section-head"><h2>精选作曲家</h2><span class="tag">Featured · ${feat.length} 位</span></div>
  ...
  function home(){
    ...
    const tl = ERAS.map(e=>{
      ...
      return `<div class="tl-card" onclick="location.hash='#/era/${e.key}'">
        ...
        <div class="nm">${esc(e.name.split(" / ")[1]||e.name)}</div>
    ...
    return `
    <section class="hero">
      <div class="kicker">从素歌到简约主义 · 从蜡筒录音到流媒体</div>
      <h1>一千零一张<br><em>古典</em>必听录音</h1>
      <p class="lead">按时期、体裁、作曲家与心情进入九百年的古典音乐史。每一张标杆录音旁边，是一段中文导读——告诉你它为什么重要、该从哪听起。</p>
      <div class="stat-row">
        <div class="stat"><b>${ALBUMS.length}</b><span>张录音</span></div>
        <div class="stat"><b>${ERAS.length}</b><span>历史时期</span></div>
        <div class="stat"><b>${new Set(ALBUMS.map(a=>a.artist)).size}</b><span>位作曲家</span></div>
        <div class="stat"><b>${window.MOODS.length}</b><span>心情入口</span></div>
      </div>
    </section>

    <section class="section">
      <div class="section-head"><h2>时期时间轴</h2><span class="tag">Timeline · ${ERAS.length} eras</span></div>
    ...
    <section class="section">
      ...
      <div class="today">
        ...
        <div class="t-body">
          <h3>${esc(workZh(pick))}</h3>
          <div class="t-artist">${esc(composerZh(pick.artist))} · ${esc(pick.perf)} · ${pick.year} · ${esc(pick.label)}</div>
          <p>${esc(pick.reason)}</p>
  ...
  function erasPage(){
    ...
    return `${crumb()}<div class="section-head"><h2>按时期进入</h2><span class="tag">${ERAS.length} Eras</span></div>
  ...
  function eraPage(key){
    ...
    return `${crumb()}
      ...
      <div class="section-head"><h2>本时期录音</h2><span class="tag">${list.length} recordings</span></div>
      ${grid(list)}`;
  }
  ...
  function genresPage(){
    ...
    return `${crumb()}<div class="section-head"><h2>按体裁进入</h2><span class="tag">Genres</span></div>
      <div class="tile-grid">${tiles}</div>`;
  }
  ...
  function artistsPage(){
    ...
    // 作曲家按生年排序（有小传者），无生年退到末尾按字母
    const ordered=Object.keys(m).sort((a,b)=>{
      const ba=BIOS[a],bb=BIOS[b];
      const ya=ba&&ba.born, yb=bb&&bb.born;
      if(ya&&yb) return ya-yb; if(ya)return -1; if(yb)return 1; return a.localeCompare(b);
    });
      ...
      return `<div class="tile${b?" has-bio":""}" data-search="${esc(search)}" onclick="location.hash='#/artist/${enc(a)}'">
        <div class="tile-head">
          ...
          <div class="tile-head-txt">
            <div class="t-k">COMPOSER${b?'<span class="bio-badge">小传</span>':""}</div>
            <h3>${esc(a)}</h3>
          </div>
        </div>
    ...
    return `${crumb()}<div class="section-head"><h2>按作曲家进入</h2><span class="tag">${ordered.length} 位 · ${bioN} 篇小传</span></div>
      <div class="artist-filter-bar">
        <input id="artistFilter" type="search" placeholder="筛选作曲家 / 译名…" autocomplete="off" aria-label="筛选作曲家">
        <span class="aff-count"><b id="artistShown">${ordered.length}</b> / ${ordered.length} 位</span>
      </div>
      <div class="tile-grid" id="artistGrid">${tiles}</div>
      <p class="empty" id="artistEmpty" style="display:none">没有匹配的作曲家。</p>`;
  }
  function artistPage(a){
    ...
    const b=BIOS[a];
    const head=b
      ? `<div class="section-head"><h2>全部录音</h2><span class="tag">${list.length} recordings · 按年份</span></div>`
      : `<div class="section-head"><h2>${esc(a)}</h2><span class="tag">${list.length} recordings</span></div>`;
  ...
  function instrumentsPage(){
    const m={};ALBUMS.forEach(a=>a.instruments.forEach(i=>m[i]=(m[i]||0)+1));
    const order=window.INSTRUMENTS.filter(i=>m[i]);
    Object.keys(m).forEach(i=>{ if(!order.includes(i)) order.push(i); });
    const tiles=order.map(i=>
      `<div class="tile" onclick="location.hash='#/instrument/${enc(i)}'"><div class="t-k">FORCES</div>
        <h3>${esc(i)}</h3><div class="cnt">${m[i]} 张 →</div></div>`).join("");
  ...
  function albumPage(id){
    ...
    const related=ALBUMS.filter(x=>x.id!==a.id &&
      (x.artist===a.artist || x.era===a.era || x.genres.some(g=>a.genres.includes(g))))
    ...
    <div class="detail">
      ...
      <div>
        ...
        <h1>${esc(workZh(a))}</h1>
        <div class="d-worken">${esc(a.title)}</div>
        <div class="d-artist"><a href="#/artist/${enc(a.artist)}">${esc(composerZh(a.artist))}</a><span class="d-artist-zh">${esc(a.artist)}</span></div>
        ...
        <div class="tags"><span class="muted" style="font-family:var(--label);font-size:.78rem;align-self:center">编制：</span>${tagLinks(a.instruments,"instrument")}</div>
  ...
  function aboutPage(){
    ...
    <div class="about">
      <header class="about-hero">
        ...
        <h1>一千零一张古典必听录音地图</h1>
        <p class="lead">从中世纪的修道院素歌到当代的简约脉动，从蜡筒录音到流媒体——一座可听、可看、可读的古典音乐地图。每一张标杆录音旁边，是一段告诉你"它为什么重要、该从哪听起"的中文导读。</p>
        <div class="about-stats">
          <div><b>${albumN}</b><span>张录音</span></div>
          <div><b>${eraN}</b><span>历史时期</span></div>
          <div><b>${artistN}</b><span>位作曲家</span></div>
          <div><b>${genreN}</b><span>个体裁</span></div>
        </div>
      </header>

      <section class="about-sec">
        <h2>这是什么</h2>
        <p>1001 Classical 是一个古典音乐<strong>策展</strong>项目，不是播放器。它的灵感来自 Matthew Rye 主编的《1001 Classical Recordings You Must Hear Before You Die》——一本以"录音"而非"作品"为单位的鉴赏指南：同一部《第九交响曲》，富特文格勒、卡拉扬、伯恩斯坦的版本各有其不可替代之处。本站精选古典音乐史上公认的<strong>标杆与传奇录音</strong>，每张配中文导读、时期、体裁、编制与心情标签，帮你找到下一张该听的唱片。</p>
      </section>
      ...
      <section class="about-sec">
        <h2>作曲家小传</h2>
        <p>点进任一<a href="#/artists">作曲家</a>，页首是一张中文小传卡：译名、生卒、国别、身份，以及一段"为什么重要"，配一张来自维基百科的肖像；下方按年份列出这位作曲家的入选录音。全站共收录 <strong>${artistN}</strong> 位作曲家的小传，覆盖全部 ${albumN} 张录音。作曲家索引页支持按原名或中文译名<strong>即时筛选</strong>，全库搜索也认中文译名（搜"贝多芬""柴科夫斯基"即可）。</p>
      </section>
      ...
      <section class="about-sec dev">
        <h2>开发者</h2>
        <p>由 <strong>xujiann</strong> 设计与开发，是"1001 × 1001"收藏计划的一员。问题反馈、纠错或建议，欢迎来信：<a href="mailto:popstudy@gmail.com">popstudy@gmail.com</a>。</p>
      </section>
    </div>`;
  }
